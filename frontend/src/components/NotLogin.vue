...
<template>
  ...
  <div class="fixed inset-0 flex items-center justify-center z-20">
    <div class="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full text-center relative">
      ...
      <p class="text-gray-600 mb-4">Войдите или зарегистрируйтесь, чтобы продолжить.</p>
      <button @click="showLogin = true" class="w-full bg-blue-500 text-white py-2 rounded-md mb-2 hover:bg-blue-600 cursor-pointer">
        Вход
      </button>
      <button @click="showRegister = true" class="w-full bg-green-500 text-white py-2 rounded-md hover:bg-green-600 cursor-pointer">
        Регистрация
      </button>
    </div>
  </div>
  ...
  <div v-if="showLogin" class="fixed inset-0 flex items-center justify-center z-40">
    <div class="bg-white p-6 rounded-lg shadow-lg relative max-w-md w-full">
      <button @click="closeModal" class="absolute top-2 right-2 text-gray-600 text-xl cursor-pointer">&times;</button>
      <LoginForm />
    </div>
  </div>
